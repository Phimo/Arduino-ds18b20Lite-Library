...
{
	errorMsg = 0;
	if(_resolution < 9 || _resolution > 12)
	{
		errorMsg = DS_INVALID_RESOLUTION;
		return false;
	}
		resolution = _resolution;
	_wire->reset();
	_wire->search(dsAddress);
	if(_wire->crc8(dsAddress, DS_ADDRESS_CRC_REG) != dsAddress[DS_ADDRESS_CRC_REG])
	{
		errorMsg = DS_BAD_ADD_CRC;
		return false;
	}
	if (dsAddress[0] != 0x28)
	{
		errorMsg = DS_WRONG_FAMILY;
		return false;
	}
	else
	{
...
uint8_t ds18b20Lite::read()
{
  ...
  while(_wire->crc8(scratchPad, DS_SCRATCHPAD_CRC_REG) != scratchPad[DS_SCRATCHPAD_CRC_REG])
  {
	...
	_wire->reset();
    delay(0);
    if (millis() - timeOutMillis > DS_TIMEOUT) return false;
  }
