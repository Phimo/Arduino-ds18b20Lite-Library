...
public:
	...
	// start the DS18B20 and set the resolution, returns true if successful or false if not
	// errorMsg contains the reason for failure is applicable or 0
	uint8_t begin(uint8_t _resolution = DS_RES_12);
	
	// read the DS18B20 temperature and store it in tempC and tempF as a float
	...
	// the current resolution of the DS18B20. Valid values are 9, 10, 11 or 12
 	uint8_t resolution, errorMsg;
	
	// variables for the temperature for the user to read
	// previous readings remain unaltered util a new successful read
	float tempC, tempF;
